import { useGetMe, useGetDashboardSummary, useListSubjects } from "@workspace/api-client-react";
import { Link, useLocation } from "wouter";
import { motion } from "framer-motion";
import { BookOpen, Video, FileText, CheckCircle2, PlayCircle, Clock } from "lucide-react";
...
export default function Dashboard() {
  ...
  const gradeBadge =
    grade === "troisieme" ? "B" : grade === "deuxieme" ? "2" : "1";
    ...
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <section className="bg-gradient-to-r from-primary to-emerald-800 rounded-2xl p-4 sm:p-6 md:p-8 text-primary-foreground shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
          ...
          <div
            className="bg-white/10 backdrop-blur-md border border-white/20 p-3 sm:p-4 rounded-2xl flex items-center gap-3 sm:gap-4 text-left w-full sm:w-auto"
            dir="ltr"
          >
            <div>
              <p className="text-xs text-primary-foreground/70 font-semibold uppercase tracking-wider">
                Niveau
              </p>
              <p className="font-bold text-lg">{gradeLabel}</p>
            </div>
        ...
        <Card className="border-none shadow-sm hover-elevate">
          <CardContent className="p-3 sm:p-4 flex flex-col items-center justify-center text-center space-y-1">
            ...
            {isLoading ? (
              ...
              <p className="text-2xl font-bold leading-none">
                {summary?.recentLessons?.filter((l) => l.type === "video").length || 0}
              </p>
            )}
        ...
        <Card className="border-none shadow-sm hover-elevate">
          <CardContent className="p-3 sm:p-4 flex flex-col items-center justify-center text-center space-y-1">
            ...
            {isLoading ? (
              ...
              <p className="text-2xl font-bold leading-none">
                {summary?.recentLessons?.filter((l) => l.type === "pdf").length || 0}
              </p>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <section className="lg:col-span-2 space-y-3">
          ...
          {!validBranchId ? (
            <div
              className="bg-card rounded-2xl border-2 border-dashed border-border/50 p-8 text-center"
              dir="rtl"
            >
            ...
            <div className="bg-card rounded-2xl border p-8 text-center" dir="rtl">
              ...
              {subjects.map((subject, index) => (
                ...
                >
                  <Link href={`/subjects/${subject.id}`}>
                    <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-border bg-card transition-all cursor-pointer group h-24 sm:h-28 p-2 sm:p-3 text-center hover:border-primary/50 hover:shadow-md">
                      <div
        ...
        <section className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link href="/lessons" className="text-primary text-sm font-semibold hover:underline">
              الكل
            </Link>
          </div>
                    ...
                    <div className="flex gap-3 p-3 bg-card rounded-xl shadow-sm border border-border/50 hover-elevate transition-all group cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          {lesson.subjectName || lesson.subjectId}
                        </p>
